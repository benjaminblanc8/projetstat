...
```{r, label="fig-1", fig.cap="Classement des pays de l'OCDE", echo=FALSE, message=FALSE, warning=FALSE, fig.width=7, fig.height=5}
...
selection <- bind_rows(top3, bottom3, france) %>%
  ...
  mutate(groupe = case_when(
    ...
    mDim1 > 0.5  & mDim2 > 0.5    ~ "Modèles anglophones\net nordiques",
    mDim1 > 0.5  & mDim2 < -0.5   ~ "Systèmes exigeants\nà capital élevé",
    TRUE                            ~ "Europe continentale\net Amérique du Sud"
  ))
...
ggplot(selection, aes(x = reorder(CNT, score_moyen),
  ...
  labs(
    title   = "Classement des pays : Top 3, Bottom 3 et France",
    x       = NULL,
...
```{r, label="fig-2", echo=FALSE,message=FALSE,warning=FALSE}
...
```{r, label="fig-3", fig.cap="Lien entre statut socio-économique (ESCS) et score global", echo=FALSE, fig.width=7, fig.height=5, message=FALSE, warning=FALSE}
...
ggplot(pisa_oecd_clean %>% filter(ESCS > -4 & ESCS < 3),
  ...
  labs(
    ...
    fill = "Densité",
    caption = "Source : PISA 2022 — OCDE. Champ : élèves de 15 ans des pays membres de l'OCDE.\nNote de lecture : les zones plus foncées indiquent une plus forte concentration d'élèves."
  ) +
  theme_minimal() +
  theme(
    plot.title      = element_text(face = "bold", size = 12),
...
```{r, label="fig-4", fig.cap="Corrélation entre variables quantitatives économique et scores", echo=FALSE, fig.width=8, fig.height=5, message=FALSE, warning=FALSE}
# ── Heatmap corrélations avec le score global ──────────────────────────
...
# Renommage du score
mat_r_long$score <- recode(
  mat_r_long$score,
  score_global = "Score global"
)

# Graphique
ggplot(mat_r_long, aes(x = score, y = label_var, fill = r)) +
  ...
  labs(
    title = "Corrélation entre variables socio-économiques et score global",
    x = NULL,
    y = NULL,
...
```{r, label="fig-5", fig.cap="Force du lien entre variables qualitatives de nature économique et scores scolaires", echo=FALSE, fig.width=8, fig.height=6, message=FALSE, warning=FALSE}
...
ggplot(mat_eta_long, aes(x = matiere, y = label_var, fill = eta)) +
  ...
  labs(
    title = "Force du lien entre variables qualitatives de nature économique et score",
    x = NULL, y = NULL,
...
```{r, label="fig-6", fig.cap="Corrélation entre variables quantitatives de nature scolaire et score scolaire", echo=FALSE, fig.width=8, fig.height=6, message=FALSE, warning=FALSE}
...
ggplot(mat_r_long, aes(x = matiere, y = label_var, fill = r)) +
  ...
  labs(
    title = "Corrélation entre variables quantitatives de nature scolaire et score",
    x = NULL, y = NULL,
    caption = "Source : PISA 2022 — OCDE. Champ : élèves de 15 ans des pays membres de l'OCDE.\nNote de lecture : r proche de 1 indique un lien positif fort, proche de -1 un lien négatif fort, proche de 0 l'absence de lien linéaire."
  ) +
  theme_minimal(base_size = 11) +
...
```{r, label="fig-7", fig.cap="Force du lien entre variables qualitatives de nature scolaire et scores scolaires", echo=FALSE, fig.width=8, fig.height=6, message=FALSE, warning=FALSE}
# ── Heatmap eta (variables qualitatives / ordinales) ────────────────────────
labels_eta <- c(
...
```{r, label="fig-8", fig.cap="Corrélation entre variables quantitatives et scores scolaires", echo=FALSE, message=FALSE, warning=FALSE}
...
```{r, label="fig-9", fig.cap="Force du lien entre variables ordinales/qualitatives et scores scolaires", echo=FALSE, message=FALSE, warning=FALSE}
# ── Heatmap eta (variables qualitatives / ordinales) ────────────────────────
labels_eta <- c(
...
ggplot(mat_eta_long, aes(x = matiere, y = label_var, fill = eta)) +
  ...
  labs(
    title = "Force du lien = variables ordinales/qualitatives et score )",
    x = NULL, y = NULL,
    caption = "Source : PISA 2022 — OCDE. Champ : élèves de 15 ans des pays membres de l'OCDE.\nNote de lecture : η (eta) mesure l'intensité du lien entre une variable qualitative et une variable quantitative. Il varie entre 0 (aucun lien) et 1 (lien parfait)."
  ) +
  theme_minimal(base_size = 11) +
...
```{r, label="fig-10", fig.cap="Score moyen par matière selon le genre", echo=FALSE, fig.width=6, fig.height=4, message=FALSE, warning=FALSE}
pisa_oecd_clean %>%
...
```{r, label="fig-11", fig.cap="Score moyen global selon le statut migratoire", echo=FALSE}
pisa_oecd_clean %>%
  filter(!is.na(IMMIG)) %>%
  group_by(IMMIG) %>%
...
```{r, label="fig-12", fig.cap="score moyen selon redoublement", echo=FALSE}
pisa_oecd_clean %>%
...
```{r, label="fig-13", fig.cap="score moyen selon les absences prolongées", echo=FALSE}
pisa_oecd_clean %>%
...
```{r, label="fig-14", fig.cap="statistiques des scores par pays", echo=FALSE, message=FALSE, warning=FALSE}
...
kable(moyennes_groupe_b, digits = 2,
      caption = "Moyennes des variables d’environnement scolaire par groupe de pays",
      booktabs = TRUE) %>%
  kable_styling(latex_options = c("hold_position", "scale_down"), font_size = 8) %>%
...
# Variables de la partie B (sans les 2 dernières)
vars_b <- intersect(
  c("PQSCHOOL", "SCHSUST", "ST272Q01JA", "ST059Q01TA",
    "ST059Q02JA", "EXPO21ST"),
  names(pisa_oecd_clean)
...
ggplot(diff_std,
  ...
  scale_fill_manual(values = c("TRUE" = "#1a9850", "FALSE" = "#d73027"),
                    guide = "none") +
  labs(
    title = "Écart standardisé entre pays forts et faibles",
    x = NULL,
    y = "Différence standardisée (Fort - Faible)",
...
# Moyenne des heures de cours par pays
...
# Boxplot + points + labels
ggplot(heures_pays, 
       aes(x = moyenne, y = heures_moy, color = groupe)) +
  geom_point(size = 2) +
...
# Nuage de points
ggplot(hisced_pays,
       aes(x = hisced_mode, y = score_moyen, color = groupe)) +
  geom_point(size = 2) +
  geom_text_repel(aes(label = CNT), size = 3) +
  ...
  labs(
    title = "Lien entre niveau d’éducation parental (mode) et performance des pays",
    x = "Niveau d’éducation parental le plus fréquent (HISCED)",
    y = "Score moyen global",
    color = "Groupe de pays",
    caption = "Source : PISA 2022 — OCDE"
  ) +
...
ggplot(immig_pays, 
  ...
  labs(
    title = "Lien entre proportion d’élèves issus de l’immigration et performance des pays",
...
```{r, label="fig-25", fig.cap="Éboulis des valeurs propres", echo=FALSE, message=FALSE, warning=FALSE, fig.width=7, fig.height=4}
...
```{r, label="fig-26", fig.cap="Cercle des corrélations — plan Dim.1 × Dim.2", echo=FALSE, message=FALSE, warning=FALSE, fig.width=7, fig.height=6}
...
```{r, label="fig-28", fig.cap="Contribution des variables aux axes 1 et 2", echo=FALSE, message=FALSE, warning=FALSE, fig.width=7, fig.height=4}
...
```{r, label="fig-29", fig.cap="Qualité de représentation des pays (cos²)", echo=FALSE, message=FALSE, warning=FALSE, fig.width=8, fig.height=5}
...
```{r, label="fig-30", fig.cap="Lien entre l'axe 1 de l'ACP et le score moyen national", echo=FALSE, message=FALSE, warning=FALSE, fig.width=8, fig.height=5}
