...
ggplot(pisa_oecd_clean %>% filter(ESCS > -4 & ESCS < 3),
  ...
  labs(
    ...
    caption = "Source : PISA 2022 — OCDE. Champ : élèves de 15 ans des pays membres de l'OCDE.\nNote de lecture : La concentration d’élèves la plus élevée se situe autour d’un indice ESCS compris entre −0,5 et 0,5, associé à un\n score global proche de 500 points."
  ) +
  theme_minimal() +
  theme(
    plot.title   = element_text(face = "bold", size = 12),
...
```{r, label="fig-4", fig.cap="Corrélation entre variables quantitatives économique et score global", echo=FALSE, fig.width=8, fig.height=5, message=FALSE, warning=FALSE}
# ============================================================
...
fig3 <- ggplot(data_lollipop, aes(x = reorder(variable, r), y = r)) +
  ...
  ylim(0, 0.58) +
  labs(
    title    = "Corrélation entre variables économiques et score global",
    subtitle = "Coefficient de corrélation de Pearson (r)",
    x        = NULL, 
    y        = "r de Pearson",
    caption  = paste0(
...
ggplot(mat_eta_long, aes(x = matiere, y = label_var, fill = eta)) +
  ...
  labs(
    x = NULL, y = NULL,
    caption = "Source : PISA 2022 — OCDE. Champ : élèves de 15 ans des pays membres de l'OCDE.\nNote de lecture : 0,45 : le nombre de livres à la maison est associé à 45% des différences de réussite scolaire."
  ) +
...
```{r, label="fig-8", fig.cap="Variables scolaires et lien avec le score global", echo=FALSE, fig.width=10, fig.height=8, message=FALSE, warning=FALSE}
# ============================================================
# FIGURES 5 + 6 → Cleveland dot plot avec gradient
# ============================================================
...
data_cleveland <- data.frame(
  variable = c(
    ...
    "Durée préscolarisation",
    "Soutien émotionnel enseignants",
...
fig56 <- ggplot(data_cleveland, 
  ...
  geom_segment(aes(x = 0, xend = coeff,
                   ...
                   color = coeff),
               linewidth = 1.2) +
  geom_point(aes(color = coeff, shape = type), size = 5) +
  geom_text(
    aes(label = round(coeff, 2)),
    hjust    = ifelse(data_cleveland$coeff >= 0, -0.5, 1.5),
    size     = 3.3,
    fontface = "bold"
  ) +
  scale_color_gradient2(
  ...
  scale_shape_manual(
    values = c("eta" = 17, "r" = 16),
    labels = c("eta" = "Coefficient η (eta)", 
               "r"   = "Corrélation de Pearson (r)"),
    name   = "Type de mesure"
  ) +
  xlim(-0.18, 0.65) +
  labs(
    x        = "Coefficient de liaison", 
    y        = NULL,
    caption  = paste0(
      "Source : PISA 2022 – OCDE. ",
      "Champ : élèves de 15 ans des pays membres de l'OCDE.\n",
      "Note de lecture : le type de filière (PROGN) explique 49 % des ",
      "différences de réussite scolaire (η = 0,49)"
    )
  ) +
  theme_minimal(base_size = 12) +
  theme(
...
```{r, label="fig-9", fig.cap="Corrélation entre variables quantitatives et scores scolaires", echo=FALSE, message=FALSE, warning=FALSE}
...
```{r, label="fig-10", fig.width=6, fig.height=4, echo=FALSE, message=FALSE, warning=FALSE }
...
```{r, label="fig-11", fig.cap="Tableau des 5 vairables les plus liées au score global", echo=FALSE, fig.width=8, fig.height=6, message=FALSE, warning=FALSE}
# ─────────────────────────────────────────────────────────────
